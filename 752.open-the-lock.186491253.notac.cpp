...
class Solution {
public:
void openLockDFS(unordered_set<string>& deadstates, string& target, string& curLock, int& pathLength, int& globalPathlength)
{
	if (curLock.size() == 4)
	{
		if (target.compare(curLock) == 0)
			globalPathlength = min(globalPathlength, pathLength);
		return;
	}
	...
	for (int i = 0; i < 9; i++)
	{
		...
		if (curLock.size() == 4 && deadstates.count(curLock)) return;
		else
		{
			pathLength++;
			openLockDFS(deadstates, target, curLock, pathLength, globalPathlength);
			curLock.pop_back();
			--pathLength;
		}
	}
...
int openLock(vector<string>& deadends, string target) 
{
	...
	int globalPathlength = INT_MAX;
	openLockDFS(deadstates, target, curLock, pathLength, globalPathlength);
	return globalPathlength == INT_MAX ? -1 : globalPathlength;
