...
public:
 ...
 {
	 if (!indexes.size()) return S;
     
	 map<int, pair<int,string>> replacemap;

	 for (int i = 0; i < indexes.size(); i++)
	 {
		 if (S.find(sources[i], indexes[i]) == indexes[i]) replacemap[indexes[i]] = {sources[i].size(), targets[i] };
	 }

	 for (int i = S.size(); i >= 0; i--)
	 {
		 if (replacemap.find(i) != replacemap.end())
			 S.replace(i, replacemap[i].first, replacemap[i].second);
	 }
	 return S;
 }
};
