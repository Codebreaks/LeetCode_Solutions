...
{
	int totalpsntime = 0;
	if (!timeSeries.size()) return totalpsntime;
	int poisonending = timeSeries[0];
    
    int offset = 0;

	for (int i = 0; i < timeSeries.size(); i++)
	{
		offset = (poisonending - timeSeries[i]) > 0? poisonending - timeSeries[i] : 0;
        
        totalpsntime = totalpsntime + duration - offset;
        
		poisonending = timeSeries[i] + duration;
	}
	return totalpsntime;
}
