...
bool canCrossUtil(vector<int>& stones, int pos, int k)
{
	string key = "pos" + to_string(pos) + "k" + to_string(k);

	if (paths.find(key) != paths.end()) return paths.find(key)->second;
	...
	for (int i = pos + 1; i < stones.size(); i++)
	{
		...
		if (gap > k + 1)
		{
			paths.insert(pair<string, bool> (key, false));
            			return false;

		}
		if (canCrossUtil(stones, i, gap)) return true;
	}
