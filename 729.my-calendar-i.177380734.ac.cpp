...
{
	if (calendar.empty()) {calendar[start] = end; return true;}
	else
	{
		itr = calendar.rbegin();

		while (itr != calendar.rend())
		{
			if (itr->first > start)
				itr++;

			else if (start >= itr->second && (itr == calendar.rbegin()))
			{
				calendar[start] = end;
				return true;
			}
            
			else if (itr->first == start) return false;

			else if (start < itr->second) return false;

			else
			{
				--itr;
				if (end > itr->first) return false;
